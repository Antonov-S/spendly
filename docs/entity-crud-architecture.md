...
| Action | Description |
|---|---|
| `createFinancialAccount` | Create with name, type, currency, startingBalance, optional color/icon |
...
Zod schema validates: `name` (non-empty string), `type` (`AccountType` enum), `currency` (3-char ISO code), `startingBalance` (Decimal ≥ 0).
