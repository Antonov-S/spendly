...
export function AccountEmptyState({ onCreate }: AccountEmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-line bg-surface px-6 py-16 text-center">
      ...
      <p className="mt-1.5 max-w-xs text-[12px] text-ink-2">
        Add a checking account, cash envelope, or credit card with its starting
        balance — every transaction lives in an account.
      </p>
      <button
        type="button"
        onClick={onCreate}
